...
test_that("cleans NA values", {
  ...
  fixed <- clustsimlR:::cleanMissingData(missing_data_NA)
...
test_that("cleans NaN values", {
    ...
    fixed <- clustsimlR:::cleanMissingData(missing_data_NaN)
...
test_that("cleans NA and NaN values", {
    ...
    fixed <- clustsimlR:::cleanMissingData(missing_data_NA_NaN)
...
test_that("leaves data unchanged if no NA or NaN", {
    ...
    fixed <- clustsimlR:::cleanMissingData(ok_df)
...
test_that("cleans data with whole NA columns", {
    ...
    missing_data_whole_col_NaN <- data.frame(c(1, 2, 3), c(4, 5, 6),
                                             c(NaN, NaN, NaN))
    fixed <- clustsimlR:::cleanMissingData(missing_data_whole_col_NA)
    fixed2 <- clustsimlR:::cleanMissingData(missing_data_whole_col_NaN)
